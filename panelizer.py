...
def duplicate_board_items(board, items, num_x, num_y, board_width, board_height, create_copy=None):
    """
...
def create_panel_outline(board, array_center, array_width, array_height,
                         h_rail_width, v_rail_width, padding, layer):
    """
...
def add_vscores(board, layertable, panel_center, panel_width, panel_height,
                board_width, board_height, num_x, num_y,
                h_rail_width, v_rail_width, vscore_extend,
                vscore_layer, vscore_text_layer, vscore_text):
    ...
    for x in x_range:
        ...
        line = create_vscore_line(board, x_loc, vscore_top, x_loc, vscore_bottom,
                                  layertable[vscore_layer])
        v_scores.append(line)
        create_vscore_text(board, vscore_text, x_loc, vscore_top - V_SCORE_TEXT_SIZE * SCALE,
                           900, pcbnew.GR_TEXT_H_ALIGN_LEFT, layertable[vscore_text_layer])
    ...
    for y in y_range:
        ...
        line = create_vscore_line(board, vscore_left, y_loc, vscore_right, y_loc,
                                  layertable[vscore_layer])
        v_scores.append(line)
        create_vscore_text(board, vscore_text, vscore_left - V_SCORE_TEXT_SIZE * SCALE, y_loc,
                           0, pcbnew.GR_TEXT_H_ALIGN_RIGHT, layertable[vscore_text_layer])
...
def parse_args():
    ...
    parser.add_argument("--padding", type=int, default=1, help="Extra space between boards")
...
def validate_args(args):
    ...
    if ((args.hrailtext or args.htitle) and args.hrail < MIN_RAIL_WIDTH_FOR_TEXT) or \
       ((args.vrailtext or args.vtitle) and args.vrail < MIN_RAIL_WIDTH_FOR_TEXT):
        print(f"Rail width must be at least {MIN_RAIL_WIDTH_FOR_TEXT}mm if using rail text. Quitting.")
        sys.exit(1)
...
def main():
    ...
    duplicate_board_items(board, board.GetTracks(), num_x, num_y, board_width, board_height)
    duplicate_board_items(board, board.GetDrawings(), num_x, num_y, board_width, board_height)
    ...
    create_panel_outline(board, array_center, array_width, array_height,
                         h_rail_width, v_rail_width, padding, layertable["Edge.Cuts"])
    ...
    vscore_bottom = add_vscores(
        board, layertable, panel_center, panel_width, panel_height,
        board_width, board_height, num_x, num_y,
        h_rail_width, v_rail_width, vscore_extend,
        vscore_layer, vscore_text_layer, vscore_text
    )
    ...
    # Add report text
    report_args = f"{output_file} ({num_x}x{num_y} panel) generated with:\n./panelizer.py"
    ...
    if panel_width / SCALE < MIN_PANEL_SIZE_MM or panel_height / SCALE < MIN_PANEL_SIZE_MM:
